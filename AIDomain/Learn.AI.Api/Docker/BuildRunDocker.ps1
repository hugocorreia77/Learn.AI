...
docker run -dt -p 5000:8080 -p 5001:8081 -e ASPNETCORE_ENVIRONMENT=$AspnetCoreEnvironment -e ASPNETCORE_HTTP_PORTS:8080 -e ASPNETCORE_HTTPS_PORTS=8081 -e ASPNETCORE_Kestrel__Certificates__Default__Password="hugocorreia77" -e ASPNETCORE_Kestrel__Certificates__Default__Path=/https/aspnetapp.pfx -v $env:USERPROFILE\.aspnet\https:/https/ --name $ContainerName --network learning-network $ImageName 
